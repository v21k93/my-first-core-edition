...
class CastAll_Comandscript : public CommandScript
{
    public:
		...
		static bool HandleUnAuraAllCommand(ChatHandler* handler, const char* args)
        {
			...
			bool triggered = (trig_str != NULL);
			sObjectAccessor->UnAuraAllPlayers(spell);
            return true;
        }
        ChatCommand* GetCommands() const
        {
            static ChatCommand CastAllCommandTable[] =
            {
				...
				{ "unauraall",        SEC_MODERATOR,         true,   &HandleUnAuraAllCommand,      "", NULL },
